...
| GET    | `/auth/me`                                                       | Validate the current teacher session                                |
...
Teacher credentials are stored in `teachers.json` and are required for signup and
unregister operations. The browser keeps the authenticated session only for the
current browser tab.
